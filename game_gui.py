...
class Board(object):
    ...
    EMPTY_SLOT = 0
    # an integer flag to represent the player 1
    PLAYER1 = 1
    # an integer flag to represent the player 2
    PLAYER2 = 2
    ...
    def terminal(self):
        # check if the game is terminal. terminal means draw or someone wins
    ...
    def row(self, r):
        # get the specific row of the game described using PLAYER1, PLAYER2 and EMPTY_SLOT
        return [e for e in self._board[r]]
    
    def col(self, c):
        # get a specific column of the game board
        return [self._board[r][c] for r in range(self.rows)]

    def dump(self, indent=0):
        # a string to describe the game board using PLAYER1, PLAYER2 and EMPTY_SLOT
...
class GUI(tk.Frame):
    ...
    def __init__(self, alg_fn_map, master=None):
        ...
        self.master.title("Adversarial Search")
    ...
    def new_game(self):
        ...
        listbox_player1 = tk.OptionMenu(self.new_game_window, player1_var, "AI-Agent", "Human", "Random")
        listbox_player2 = tk.OptionMenu(self.new_game_window, player2_var, "AI-Agent", "Human", "Random")
        ...
        player1_var.set("AI-Agent")
        player2_var.set("AI-Agent")
        ...
        player1_var.trace("w",
            lambda *args: listbox_player1_depth.config(state=tk.NORMAL if player1_var.get() == "AI-Agent" else tk.DISABLED)
        )
        player2_var.trace("w",
            lambda *args: listbox_player2_depth.config(state=tk.NORMAL if player2_var.get() == "AI-Agent" else tk.DISABLED)
        )
    ...
    def draw_checker(self, player, x, y,tag=""):
        c = "orange" if player == self.PLAYER1 else "green"
